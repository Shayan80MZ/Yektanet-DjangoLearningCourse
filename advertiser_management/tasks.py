...
def saveHourlyTask():
    for ad in Ad.objects.all():
        ...
        HourlyData.objects.create(ad=ad, clicksNum=clickNum, viewsNum=viewNum, date=datetime.now().replace(minute=0, second=0))
...
def saveDailyTask():
    for ad in Ad.objects.all():
        ...
        DailyData.objects.create(ad=ad, clicksNum=clickNum, viewsNum=viewNum, date=datetime.now().replace(hour=datetime.now().hour - 1, minute=0, second=0))
